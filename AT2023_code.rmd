...
## How many of the participants answered that they use *dude*, including those who answered they use none of the address terms in the option?
...
```{r}
# make a new column called 'DUDE' -- if 'dude' is includede in `AT-EVER-USE`, TRUE. Otherwise, FALSE
x <- org_1 %>% 
  mutate(DUDE = grepl("dude", org_1$`AT-EVER-USE`) == TRUE) %>% 
  group_by(`RESP-GENDER`, DUDE) %>% 
  summarize(Count = n_distinct(`RESP-ID`))
